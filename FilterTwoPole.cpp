...
void FilterTwoPole::setQ( float qualityFactor ) {
  ...
  qualityFactor = std::min(std::max( qualityFactor, (float)1e-3), (float)1e3 );
...
float FilterTwoPole::input( float drive ) {
  ...
  long now = currentTime();                      // get current time
  ...
  dt = std::min(std::max( (float)dt, 0.0f),1.0f/W0 );
...
void FilterTwoPole::test() {
  ...
  float nextupdateTime = 1e-6*float(currentTime());

  float inputValue = 0;
  FilterTwoPole osc( 0.2, 4, 0);

  while( true ) {
    float now = 1e-6*float(currentTime());
